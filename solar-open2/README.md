...
- 패치: `patch/solar_open2.py` — Global-Pruned 모델 지원 (레이어별 expert 수 상이)
...
| 타깃 | 설명 |
|---|---|
| `patch` | `$(MODEL_DIR)/patch/solar_open2.py` 를 빌드 컨텍스트(`patch/`)로 복사 |
| `build` | 이미지 빌드 (기본 타깃, `patch/solar_open2.py` 없으면 실패) |
...
4. `patch/solar_open2.py` 로 모델 코드 교체 (원본은 `.orig`로 백업, `py_compile`로 문법 검증)
5. Triton/FlashInfer 런타임 JIT 컴파일용 gcc + nvcc 12.9 설치

## 참고

- Makefile은 탭 대신 `>` 를 recipe 구분자로 사용합니다 (`.RECIPEPREFIX`). GNU make 3.82 이상 필요.
- `make patch`는 모델 디렉터리가 마운트된 환경에서만 동작합니다. 복사된 `patch/solar_open2.py`는 git으로 관리됩니다.
