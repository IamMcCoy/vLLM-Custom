...
class SolarOpen2MoE(nn.Module):
    def __init__(
        ...
        # `num_experts` is the per-layer routed-expert count for non-uniform
        # (globally pruned) checkpoints; falls back to the scalar config value.
    ...
    def __init__(
        ...
        if (
            ...
            self.mlp = SolarOpen2MoE(
                ...
                # Per-layer routed-expert count for non-uniform pruned models.
...
class SolarOpen2Model(nn.Module):
    ...
    def get_expert_mapping(self) -> list[tuple[str, str, int, str]]:
        ...
        # Use the maximum per-layer expert count so the global mapping covers
        # every expert id across all (possibly non-uniform) layers. Per-layer
        # FusedMoE modules only consume the ids present in their own shard.
